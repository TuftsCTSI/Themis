# Formalizing OMOP CDM Extensions: A Standardized Workflow

## Introduction

This document establishes a standardized workflow for developing extension tables to the OMOP Common Data Model (CDM). The workflow is based on best practices established by the OHDSI Medical Imaging Working Group, which successfully developed and proposed the Medical Imaging CDM (MI-CDM) extension. This formalized process ensures that extensions maintain consistency with OMOP CDM conventions while adequately capturing domain-specific data requirements.

## Overview of Extension Development Process

Extension tables should be developed when existing OMOP CDM tables cannot adequately represent domain-specific observational health data. The development process consists of three primary components:

1. **Structure Definition**: Detailed specification of table architecture and field conventions
2. **Semantic Representation**: Standardized vocabularies and concept mappings
3. **Validation and Documentation**: Examples, synthetic data, and use cases
...
When domain-specific concepts are not available in standard vocabularies, develop custom vocabularies using the Custom Vocabulary Builder (CVB) framework:

#### CVB Workflow for Extension Vocabularies

1. **Collaborative Validation**
   - Use Google Sheets interface for domain expert review
...
We are developing a comprehensive set of R packages to integrate extension tables seamlessly with the core OHDSI software ecosystem. These tools will enable researchers to leverage extension tables in the same manner as core OMOP CDM tables.
...
**To ensure extension tables can be supported by these packages, extensions must follow the conventions outlined in this document, including:**
...
1. Park WY, Jeon K, Schmidt TS, et al. Development of Medical Imaging Data Standardization for Imaging-Based Observational Research: OMOP Common Data Model Extension. Journal of Imaging Informatics in Medicine. 2024;37:899-908.

2. OMOP Common Data Model Conventions. https://ohdsi.github.io/CommonDataModel/dataModelConventions.html

3. Custom Vocabulary Builder (CVB). TuftsCTSI. https://github.com/TuftsCTSI/CVB

4. OHDSI FeatureExtractionForExtensions. https://github.com/OHDSI/FeatureExtractionForExtensions
